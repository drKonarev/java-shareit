...
class ItemServiceTest {
    ...
    void createComment_whenUserNotFound_thenThrow() {
        ...
        verify(bookingRepository, never()).findPastBookingByBooker_IdAndItem_Id(anyLong(), anyLong(), eq(LocalDateTime.now()));
    ...
    void createComment_whenPastBookingNotFound_thenThrowNullPointerException() {
        ...
        when(bookingRepository.findPastBookingByBooker_IdAndItem_Id(anyLong(), anyLong(), eq(LocalDateTime.now()))).thenReturn(Collections.emptyList());
    ...
    void createComment_whenEverythingIsOk() {
        when(userRepository.findById(anyLong())).thenReturn(Optional.of(user));
        when(itemRepository.findById(anyLong())).thenReturn(Optional.of(item));
        when(bookingRepository.findPastBookingByBooker_IdAndItem_Id(anyLong(), anyLong(),eq(LocalDateTime.now()))).thenReturn(Collections.singletonList(booking));
